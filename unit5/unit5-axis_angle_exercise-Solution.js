...
function fillScene() {
    ...
    // Calculate the cylinder axis direction and length
	...
	// YOUR CODE HERE
    // Function to add a cylinder to the scene
    function addCylinder(x, y, z, rotAxis, angle) 
	{
        var cyl = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, cylLength, 32), cylinderMaterial);
        cyl.matrixAutoUpdate = false;
        var axis = new THREE.Vector3().copy(rotAxis).normalize();
        var pos = new THREE.Vector3(x, y, z);
        cyl.matrix.makeRotationAxis(axis, angle).setPosition(pos);
        scene.add(cyl);
    }

    // Add the original cylinder to the scene
    addCylinder(0, 0, 0, new THREE.Vector3(1, 0, -1), theta);

    // Add three more cylinders with different positions and rotations
    addCylinder(0, 0, 0, new THREE.Vector3(-1, 0, -1), theta);
    addCylinder(0, 0, 0, new THREE.Vector3(1, 0, 1), theta);
    addCylinder(0, 0, 0, new THREE.Vector3(-1, 0, 1), theta);
}
