...
function createDrinkingBird() {
	...
	cylinder.rotation.x = 90 * Math.PI / 180.0; scene.add( cylinder ); 
	//nose 
	cylinder = new THREE.Mesh( new THREE.CylinderGeometry( 6, 14, 70, 32 ), headMaterial ); 
	cylinder.position.set( -70, 530, 0 ); 
	cylinder.rotation.z = 90 * Math.PI / 180.0; scene.add( cylinder ); 
	//eyes 
	...
	sphere = new THREE.Mesh( sphGeom, eyeMaterial ); 
	sphere.position.set( -48, 560, 0 ); 
	eye = new THREE.Object3D(); eye.add( sphere ); 
	eye.rotation.y = -20 * Math.PI / 180.0; 
	scene.add( eye );
...
// Set a translation part of the matrix
//matrix.makeTranslation(-49, 0, 0);

// Apply the matrix to the cylinder
//sphere.matrix = matrix;

// Disable auto updates so Three.js doesn't overwrite the custom matrix
